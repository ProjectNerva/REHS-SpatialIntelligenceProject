...
int main(int argc, char** argv) {
    ...
    // Initialize ORB-SLAM3 System with the Pangolin viewer enabled.
    // Requires a display (X11 forwarding / VNC / physical monitor) -- on a
    // headless SSH session with no DISPLAY set, Pangolin's window creation
    // will fail or hang at startup.
    ...
    std::cout << "Starting OAK-D stream with viewer enabled. Press Ctrl+C to stop tracking and save files." << std::endl;

    // Main tracking loop governed by the signal handler flag. Also bail out
    // if the user closes the Pangolin viewer window (SLAM.isFinished()
    // becomes true) rather than spinning forever with a dead viewer.
    while(bContinueRunning && pipeline.isRunning() && !SLAM.isFinished()) {
